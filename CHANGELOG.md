...
- `Do` now closes the response body when it returns an error, and the methods that ignore the response (`RestoreProduct`, `ManageProductLock`, `ValidateProductOrder`, `UpdateProductResourceTags`, `DeleteMCRPrefixFilterList`, `ModifyMCRPrefixFilterList`, `UpdateMCRWithAddOn`, `UpdateMCRIPsecAddOn` and `UpdateServiceKey`) now drain and close it. Previously each of these calls held its connection until the server dropped it. Lint now fails on any new call that leaves a response body unclosed.
