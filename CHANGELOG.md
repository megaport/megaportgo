...
## Changes
- `ModifyProduct` returns `ErrModifyPendingApproval` when the API creates an order approval request instead of modifying. That case used to read as a successful modify, though the API applied no field in the request. `ModifyPort`, `ModifyMCR` and `ModifyMVE` return it without waiting.
